...
var strategy = {
  ...
  update: function(candle) {
    ...
    while (this.settings.price_buffer_len < this.priceBuffer.length)
      this.priceBuffer.shift(); // remove old entries from the price buffer
  },
  ...
  check: function(candle) {
    if (this.predictionCount > this.settings.min_predictions) {
      // if nn has been trained as much as specified in settings
      if (
        'buy' === this.prevAction &&
      ...
      let signalSell =
        candle.close > this.prevPrice ||
        candle.close < this.prevPrice * this.hodle_threshold;
